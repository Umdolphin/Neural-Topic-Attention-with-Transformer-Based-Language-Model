...
# Make storage
MRD_ENEM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class Vanila_attention_model_topic(nn.Module):
    
    def __init__(self, num_classes=3, pretrained_embed=None, embedding_size=100, hidden_size=64,
                 dropout_keep_proba=0.1, max_word_num=500, batch_size=8):
        super(Vanila_attention_model_topic, self).__init__()
        ...
        self.word_embedding_valid = torch.tensor(self.pretrained_embed, dtype=torch.float32).to(device)
        self.word_embedding_pad = torch.zeros((1, self.embedding_size), dtype=torch.float32).to(device)
        self.word_embedding_mat = torch.concat([self.word_embedding_pad, self.word_embedding_valid], dim=0)

        self.BidirectionalGRUEncoder = nn.GRU(self.embedding_size, self.hidden_size, bidirectional=True, batch_first=True)

        self.init_variable = torch.zeros(self.units)
        self.u_context = torch.nn.init.trunc_normal_(self.init_variable)
        self.h1_fc = nn.Linear(self.units * 2, self.embedding_size)

        self.atten_out_bert_fc = nn.Linear(self.units * 2, 768)
        self.atten_out_dropout = nn.Dropout(self.dropout_keep_proba)
        self.atten_batch_norm = nn.BatchNorm1d(768)

    def embedding_data(self, x):
        x = torch.tensor(x).to(device)
        for index in range(self.batch_size):
            if index == 0:
                embedded_input = torch.index_select(self.word_embedding_mat, 0, x[index])
                embedded_input = embedded_input.unsqueeze(dim=0)
            else:
                append_input = torch.index_select(self.word_embedding_mat, 0, x[index])
                append_input = append_input.unsqueeze(dim=0)
                embedded_input = torch.concat([embedded_input, append_input], dim=0)
        used = torch.sign(torch.max(torch.abs(embedded_input), 2)[0])
        length = torch.sum(used, dim=1)
        length = length.type(torch.int32)

        return embedded_input

    def forward(self, rnn_x):
        # Embedding
        x = self.embedding_data(rnn_x)
        outputs = self.BidirectionalGRUEncoder(x)[0]

        h1 = torch.tanh(self.h1_fc(outputs))
        alpha = F.softmax(torch.sum(torch.multiply(h1, self.u_context), axis=-1).unsqueeze(dim=2), dim=0)
        doc_vec = torch.sum(torch.multiply(outputs, alpha), axis=1)
        doc_vec_dropped = self.dropout_doc_vec_layer(doc_vec)
        out = self.final_fc(doc_vec_dropped)

        return out
  

class Topic_ELECTRAClassifier(Vanila_attention_model_topic):
    
    def __init__(self, ELECTRA, dr_rate=None, params=None, reduced_vocab_size=2000, num_topic=50, num_classes=3,
                 pretrained_embed=None, embedding_size=100, RNN_hidden_size=64, topic_hidden_size=64, 
                 dropout_keep_proba=0.1, max_word_num=500, threshold=0.1, hidden_size=768):
        super(Topic_ELECTRAClassifier, self).__init__(num_classes=num_classes, pretrained_embed=pretrained_embed,
                                                      embedding_size=embedding_size, hidden_size=RNN_hidden_size,
                                                      dropout_keep_proba=dropout_keep_proba, max_word_num=max_word_num)
        ...
        if dr_rate:
            self.dropout_Electra = nn.Dropout(p=dr_rate)
        
        # Ensure to initialize parent class with parameters
        # Vanila_attention_model_topic.__init__(self, num_classes, pretrained_embed, embedding_size, RNN_hidden_size, dropout_keep_proba, max_word_num)

    def forward(self, token_ids, segment_ids, attention_mask, bow_x, rnn_x, length):
        # Get topic embeddings and variational parameters
        topic, topic_embed, mu_encoder, logvar_encoder, p_x = self.vtm.forward(bow_x)
        w = topic - self.threshold
        topic_embed_unstack = topic_embed
        topic_atten_weights = []

        # Compute RNN embeddings
        x_rnn = self.embedding_data(rnn_x)
        outputs = self.BidirectionalGRUEncoder(x_rnn)[0]
        h1 = torch.tanh(self.h1_fc(outputs))

        # Calculate attention weights for topics
        for i in range(topic_embed.size(0)):  # Use topic_embed size to avoid hardcoding num_topic
            query = topic_embed_unstack[i]
            score = torch.sum(torch.multiply(h1, query), dim=-1)
            attention_weights = F.softmax(score, dim=1).unsqueeze(2)
            topic_atten_weights.append(attention_weights)

        topic_atten_weights = torch.concat(topic_atten_weights, -1)
        w = w.unsqueeze(-1)
        topic_atten = torch.matmul(topic_atten_weights, w)
        atten_out = torch.sum(torch.multiply(topic_atten, outputs), dim=1)
        
        Topic_atten = self.atten_out_bert_fc(atten_out)
        Topic_atten_drop = self.atten_out_dropout(Topic_atten)
        Topic_atten_out = self.atten_batch_norm(Topic_atten_drop)

        # Compute ELECTRA embeddings
        ...
        if self.dr_rate:
            final_out = self.dropout_Electra(final_out)

        # Combine ELECTRA output and topic attention output
        final_weights = final_out + Topic_atten_out
        
        return self.classifier(final_weights), topic, topic_embed, mu_encoder, logvar_encoder, p_x
...
final_model = Topic_ELECTRAClassifier(Electra,reduced_vocab_size = reduced_vocab_size, num_topic = num_topic,
...
for e in range(num_epochs):
    ...
    for i, (train_input, train_label) in zip(range(train_batch_total), train_dataloader):
        ...
        out, topic, topic_embed, mu_encoder, logvar_encoder, p_x = final_model(
            token_ids, segment_ids, attention_mask, train_x_batch_bow, train_x_batch_rnn, length
        )

        kl_divergence = -0.5 * torch.sum(
            1.0 + logvar_encoder - torch.square(mu_encoder) - torch.exp(logvar_encoder), 1
        )
        ...
        if (i + 1) % train_batch_total == 0:
            print(
                "epoch {} train loss {} train acc {} train precision {} train recall {} train f1 {}".format(
                    e + 1,
                    train_ce / (i + 1),
                    train_acc / (i + 1),
                    train_precision / (i + 1),
            ...
            # train_acc_L.append(train_acc / (i + 1))
            # train_ce_L.append(train_ce / (i + 1))
    ...
    for i, (test_input, test_label) in zip(range(test_batch_total), test_dataloader):
        ...
        # y_batch = torch.tensor(Y_test[i*batch_size: (i+1)*batch_size], dtype=torch.float32)
        ...
        out, topic, topic_embed, mu_encoder, logvar_encoder, p_x = final_model(
            token_ids, segment_ids, attention_mask, test_x_batch_bow, test_x_batch_rnn, length
        )
        ...
        if (i + 1) % test_batch_total == 0:
            print(
                "epoch {} test loss {} test acc {} test precision {} test recall {} test f1 {}".format(
                    e + 1,
                    test_ce / (i + 1),
                    test_acc / (i + 1),
                    test_precision / (i + 1),
...
MRD_ENEM = MRD_ENEM.append({'Num_Topic' : num_topic,
