...
class VariationalTopicModel(nn.Module):
    ...
    def forward(self, x_bow):
        ...
        epsilon = torch.randn(logvar_encoder.size(), dtype=torch.float32).to(x_bow.device)
...
class VanilaAttentionModelTopic(nn.Module):
    def __init__(self, num_classes=3, pretrained_embed=None, embedding_size=100, 
        ...
        self.word_embedding_valid = torch.tensor(self.pretrained_embed, dtype=torch.float32).to(self.device)
        self.word_embedding_pad = torch.zeros((1, self.embedding_size), dtype=torch.float32).to(self.device)
        ...
        self.init_variable = torch.zeros(self.units).to(self.device)
        self.u_context = torch.nn.init.trunc_normal_(self.init_variable, mean=0, std=1)
        ...
        self.dropout_doc_vec_layer = nn.Dropout(self.dropout_keep_proba)
        self.final_fc = nn.Linear(self.hidden_size * 2, self.num_classes)
    ...
    def forward(self, rnn_x):
        ...
        alpha = F.softmax(torch.sum(h1 * self.u_context, axis=-1).unsqueeze(dim=2), dim=0)
        doc_vec = torch.sum(outputs * alpha, axis=1)
...
class TopicAlbertClassifier(VanilaAttentionModelTopic):
    def __init__(self, Albert, dr_rate=None, params=None, reduced_vocab_size=2000, num_topic=50, num_classes=3, 
        ...
        super(TopicAlbertClassifier, self).__init__(
    ...
    def forward(self, token_ids, segment_ids, attention_mask, bow_x, rnn_x):
        topic, topic_embed, mu_encoder, logvar_encoder, p_x = self.vtm(bow_x)
        ...
        # final_weights = pooler + Topic_atten
        ...
        if (i + 1) % train_batch_total == 0:
            print(
                "epoch {} train loss {} train acc {} train precision {} train recall {} train f1 {}".format(
                    ...
                    train_precision / (i + 1), train_recall / (i + 1), train_f1 / (i + 1)
                )
            )
        ...
        if (i + 1) % test_batch_total == 0:
            print(
                "epoch {} test loss {} test acc {} test precision {} test recall {} test f1 {}".format(
                    ...
                    test_precision / (i + 1), test_recall / (i + 1), test_f1 / (i + 1)
                )
            )
...
MRD_ANEM = MRD_ANEM.append({'Num_Topic' : num_topic,
