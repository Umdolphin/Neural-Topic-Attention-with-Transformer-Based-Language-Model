...
# Make storage
MRD_EVM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class ElectraClassifier(nn.Module):
    def __init__(self, Electra, hidden_size=768, num_classes=3, dr_rate=None, params=None):
    ...
    def forward(self, token_ids, segment_ids, attention_mask):
        # Get the embeddings from Electra
        outputs = self.Electra(
            input_ids=token_ids,
            token_type_ids=segment_ids,
            attention_mask=attention_mask
        )
        pooler_output = outputs[0]  # Assuming the first output is the hidden states
        
        # Extract the [CLS] token representation
        cls_output = pooler_output[:, 0, :]  # [batch_size, hidden_size]
...
MRD_EVM = MRD_EVM.append({'Num_Topic' : num_topic,
