...
# Make storage
News_ENEM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class CustomDataset(Dataset):
    def __init__(self, dataset, sent_idx, label_idx, bert_tokenizer, max_len, pad, pair):
        self.sentences = [bert_tokenizer(text, padding='max_length', return_tensors="pt", max_length=max_len, truncation='only_first') for text in dataset['text']]
...
class VariationalTopicModel(nn.Module):
    ...
    def forward(self, x_bow):
        ...
        epsilon = torch.randn(logvar_encoder.size(), dtype=torch.float32).to(x_bow.device)  # Use x_bow.device for compatibility
...
class Vanila_attention_model_topic(nn.Module):
    def __init__(self, num_classes=20, pretrained_embed=None, embedding_size=100, 
        ...
        self.word_embedding_mat = torch.cat([self.word_embedding_pad, self.word_embedding_valid], dim=0)

        self.BidirectionalGRUEncoder = nn.GRU(self.embedding_size, self.hidden_size, 
    ...
    def embedding_data(self, x):
        ...
        embedded_input = torch.index_select(self.word_embedding_mat, 0, x[0])
        ...
        for index in range(1, self.batch_size):
            ...
            embedded_input = torch.cat([embedded_input, append_input], dim=0)

        used = torch.sign(torch.max(torch.abs(embedded_input), dim=2)[0])
    ...
    def forward(self, rnn_x):
        ...
        doc_vec_dropped = self.atten_out_dropout(doc_vec)
        out = self.atten_batch_norm(doc_vec_dropped)

        return out


class Topic_ELECTRAClassifier(Vanila_attention_model_topic):
    def __init__(self, ELECTRA, dr_rate=None, params=None, reduced_vocab_size=2000, num_topic=50, num_classes=20, 
    ...
    def forward(self, token_ids, segment_ids, attention_mask, bow_x, rnn_x, length):
        ...
        for i in range(topic_embed.size(0)):
        ...
        topic_atten_weights = torch.cat(topic_atten_weights, dim=-1)
        w = w.unsqueeze(-1)
        ...
        final_out = torch.empty((length, 768), device=device)
...
final_model = Topic_ELECTRAClassifier(ELECTRA = Electra, dr_rate = 0.1,reduced_vocab_size = reduced_vocab_size, num_topic = num_topic, num_classes = num_classes,
                                     pretrained_embed = pretrained_embed, embedding_size = embedding_size, RNN_hidden_size = RNN_hidden_size,
                                     topic_hidden_size = topic_hidden_size, dropout_keep_proba = dropout_keep_proba,
                                     max_word_num = max_word_num, threshold = threshold
                                   ).to(device)
    ...
    for i, (train_input, train_label) in zip(range(train_batch_total), train_dataloader):
        ...
        if (i + 1) % train_batch_total == 0:
            print("epoch {} train loss {} train acc {} train precision {} train recall {} train f1 {}".format(
                e + 1, train_ce / (i + 1), train_acc / (i + 1), train_precision / (i + 1),
                train_recall / (i + 1), train_f1 / (i + 1)
            ))
    ...
    for i, (test_input, test_label) in zip(range(test_batch_total), test_dataloader):
        ...
        if (i + 1) % test_batch_total == 0:
            print("epoch {} test loss {} test acc {} test precision {} test recall {} test f1 {}".format(
                e + 1, test_ce / (i + 1), test_acc / (i + 1), test_precision / (i + 1),
                test_recall / (i + 1), test_f1 / (i + 1)
            ))
...
News_ENEM = News_ENEM.append({'Num_Topic' : num_topic,
