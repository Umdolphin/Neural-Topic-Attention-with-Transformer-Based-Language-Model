...
# Make storage
News_ENTM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class CustomDataset(Dataset):
    def __init__(self, dataset, sent_idx, label_idx, bert_tokenizer, max_len, pad, pair):
        self.sentences = [
            bert_tokenizer(text, padding='max_length', return_tensors="pt", max_length=max_len, truncation='only_first') 
...
class VariationalTopicModel(nn.Module):
    def __init__(self, vocab_size, latent_dim, num_topic, embedding_size, dropout_keep_proba=0.1):
        ...
        # Initialize weights
        # nn.init.xavier_uniform_(self.encoder_fc1.weight)
        # nn.init.xavier_uniform_(self.encoder_fc2.weight)
        # nn.init.xavier_uniform_(self.mu_fc.weight)
        # nn.init.xavier_uniform_(self.logvar_fc.weight)
        # nn.init.xavier_uniform_(self.topic_fc.weight)
...
class Topic_Paper_ELECTRAClassifier(nn.Module):
    def __init__(self, ELECTRA, reduced_vocab_size=2000, num_topic=50, num_classes=20, pretrained_embed=None,
                 dropout_keep_proba=0.1, topic_hidden_size=64, embedding_size=100, hidden_size=768,
                 threshold=0.1, dr_rate=None, params=None):
        super(Topic_Paper_ELECTRAClassifier, self).__init__()
    ...
    def forward(self, token_ids, segment_ids, attention_mask, bow_x, length):
        final_out = torch.empty(length, 768).to(token_ids.device)
        ...
        outputs = pooler
        # outputs = final_out
        h1 = torch.tanh(self.h1_fc(outputs)).to(token_ids.device)
        ...
        for i in range(self.vtm.num_topic):
            ...
            # topic_atten_weights.append(score)
...
final_model = Topic_Paper_ELECTRAClassifier(Electra,reduced_vocab_size = reduced_vocab_size, num_topic = num_topic,
...
for e in range(num_epochs):
    ...
    for i, (train_input, train_label) in zip(range(train_batch_total), train_dataloader):
        ...
        out, topic, topic_embed, mu_encoder, logvar_encoder, p_x = final_model(token_ids, segment_ids, attention_mask, train_x_batch_bow, length)
    ...
    for i, (test_input, test_label) in zip(range(test_batch_total), test_dataloader):
        ...
        length = len(token_ids)
        out, topic, topic_embed, mu_encoder, logvar_encoder, p_x = final_model(token_ids, segment_ids, attention_mask, test_x_batch_bow, length)
...
News_ENTM = News_ENTM.append({'Num_Topic' : num_topic,
