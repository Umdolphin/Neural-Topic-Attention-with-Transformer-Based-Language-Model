...
News_ANTM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class Topic_Paper_AlbertClassifier(nn.Module):
    def __init__(self, Albert, reduced_vocab_size=2000, num_topic=50, num_classes=20, pretrained_embed=None,
    ...
    def forward(self, token_ids, segment_ids, attention_mask, bow_x):
        ...
        topic_atten = torch.matmul(torch.cat(topic_atten_weights, -1), w.unsqueeze(-1))
...
News_ANTM = News_ANTM.append({'Num_Topic' : num_topic,
