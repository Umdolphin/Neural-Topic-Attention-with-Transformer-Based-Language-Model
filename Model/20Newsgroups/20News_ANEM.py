...
# Make storage
News_ANEM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class CustomDataset(Dataset):
    def __init__(self, dataset, sent_idx, label_idx, Albert_tokenizer, max_len, pad, pair):
        self.sentences = [
            Albert_tokenizer(text, padding='max_length', return_tensors="pt", max_length=max_len, truncation='only_first')
...
class Vanila_attention_model_topic(nn.Module):
    def __init__(self, num_classes=20, pretrained_embed=None, embedding_size=100,
        ...
        # Assuming final_fc is defined elsewhere or should be added
        self.final_fc = nn.Linear(self.embedding_size, self.num_classes)
    ...
    def forward(self, rnn_x):
        ...
        outputs, _ = self.BidirectionalGRUEncoder(x)
        
        h1 = torch.tanh(self.h1_fc(outputs))
        alpha = F.softmax(torch.sum(torch.multiply(h1, self.u_context), dim=-1).unsqueeze(dim=2), dim=0)
        doc_vec = torch.sum(torch.multiply(outputs, alpha), dim=1)
...
class Topic_AlbertClassifier(Vanila_attention_model_topic):
    def __init__(self, Albert, dr_rate=None, params=None, reduced_vocab_size=2000, num_topic=50, num_classes=20,
        ...
        super(Topic_AlbertClassifier, self).__init__(
            num_classes=num_classes, pretrained_embed=pretrained_embed, embedding_size=embedding_size,
            hidden_size=RNN_hidden_size, dropout_keep_proba=dropout_keep_proba, max_word_num=max_word_num
        )
        self.threshold = threshold
        self.vtm = VariationalTopicModel(
            reduced_vocab_size, topic_hidden_size, num_topic, embedding_size, dropout_keep_proba
        )
        self.Albert = Albert
        self.dr_rate = dr_rate
        self.classifier = nn.Linear(hidden_size, num_classes)
        if dr_rate:
            self.dropout_Albert = nn.Dropout(p=dr_rate)
        else:
            self.dropout_Albert = nn.Identity()  # Identity if no dropout

    def forward(self, token_ids, segment_ids, attention_mask, bow_x, rnn_x):
        # Variational Topic Model
        ...
        # Attention mechanism
        topic_atten_weights = []
        for i in range(topic_embed_unstack.size(0)):  # use size(0) to get the number of topics
        ...
        if self.dr_rate:
            out = self.dropout_Albert(pooler)
        else:
            out = pooler
...
final_model = Topic_AlbertClassifier(Albert,reduced_vocab_size = reduced_vocab_size, num_topic = num_topic,
...
for e in range(num_epochs):
    ...
    for i, (train_input, train_label) in zip(range(train_batch_total), train_dataloader):
        ...
        out, topic, topic_embed, mu_encoder, logvar_encoder, p_x = final_model(
            token_ids, segment_ids, attention_mask, train_x_batch_bow, train_x_batch_rnn
        )
    ...
    for i, (test_input, test_label) in zip(range(test_batch_total), test_dataloader):
        ...
        out, topic, topic_embed, mu_encoder, logvar_encoder, p_x = final_model(
            token_ids, segment_ids, attention_mask, test_x_batch_bow, test_x_batch_rnn
        )
...
News_ANEM = News_ANEM.append({'Num_Topic' : num_topic,
