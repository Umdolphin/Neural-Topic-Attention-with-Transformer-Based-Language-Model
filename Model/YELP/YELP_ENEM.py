...
# Make storage
YELP_ENEM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class CustomDataset(Dataset):
    def __init__(self, dataset, sent_idx, label_idx, bert_tokenizer, max_len, pad, pair):
        self.sentences = [
            bert_tokenizer(text, padding='max_length', return_tensors="pt", max_length=max_len, truncation=True)
...
class Vanila_attention_model_topic(nn.Module):
    def __init__(self, num_classes=5, pretrained_embed=None, embedding_size=100, hidden_size=64,
...
class Topic_ELECTRAClassifier(Vanila_attention_model_topic):
    def __init__(self, ELECTRA, dr_rate=None, params=None, reduced_vocab_size=2000, num_topic=50, num_classes=5,
        ...
        super(Topic_ELECTRAClassifier, self).__init__()
        ...
        if (i + 1) % train_batch_total == 0:
            print(
                "epoch {} train loss {} train acc {} train precision {} train recall {} train f1 {}".format(
                    e + 1, train_ce / (i + 1), train_acc / (i + 1),
                    train_precision / (i + 1), train_recall / (i + 1),
                    train_f1 / (i + 1)
                )
            )
        ...
        if (i + 1) % test_batch_total == 0:
            print(
                "epoch {} test loss {} test acc {} test precision {} test recall {} test f1 {}".format(
                    e + 1, test_ce / (i + 1), test_acc / (i + 1),
                    test_precision / (i + 1), test_recall / (i + 1),
                    test_f1 / (i + 1)
                )
            )
...
YELP_ENEM = YELP_ENEM.append({'Num_Topic' : num_topic,
