...
# Make storage
YELP_EVM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class CustomDataset(Dataset):
    def __init__(self, dataset, sent_idx, label_idx, Electra_tokenizer, max_len, pad, pair):
        self.sentences = [
            Electra_tokenizer(text, padding='max_length', return_tensors="pt", max_length=max_len, truncation=True)
...
class ElectraClassifier(nn.Module):
    def __init__(self,
        ...
        # Uncomment if you want to initialize weights
...
YELP_EVM = YELP_EVM.append({'Num_Topic' : num_topic,
