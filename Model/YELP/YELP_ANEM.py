...
# Make storage
YELP_ANEM = pd.DataFrame(columns = ['Num_Topic','AUC','Precision','Recall','F1','random_seed','Time'])
...
class Topic_AlbertClassifier(Vanila_attention_model_topic):
    def __init__(self, Albert, dr_rate=None, params=None, reduced_vocab_size=2000, num_topic=50, num_classes=5, 
        ...
        super(Topic_AlbertClassifier, self).__init__(
            ...
            dropout_keep_proba=dropout_keep_proba,
        ...
        # Optionally initialize parent class if needed
    ...
    def forward(self, token_ids, segment_ids, attention_mask, bow_x, rnn_x):
        ...
        if self.dr_rate:
            pooler = self.dropout_Albert(pooler)
        
        final_weights = pooler + Topic_atten_out
...
YELP_ANEM = YELP_ANEM.append({'Num_Topic' : num_topic,
